...
export function PopUpWindowCenterPosition(url, title, w, h) {
...
export function getDateFromFullDate(date, dateString) {
    let d = new Date(date);
    dateString.innerHTML = d.toDateString();
}
/* -- end -- make data update with interval*/
export function getYearFromFullDate(date, dateString) {
    let d = new Date(date);
 ...
 * */
export function setUploadProgressSystem(appenderId) {
 ...
 * @param URL string*/
export function uploadFile(ElementName, ElementID, URL) {
 ...
 * @param event any
 * */
export function completeHandler(event) {
 ...
 * */
export function previewImage(fileId, previewerId) {
 ...
 * @param callback any
 * */
export function popUpDialogBoxDriver(selector, titleText, processURL, messageView, callback) {
 ...
 * */
export function PopUpDialogBox(titleText, messageText, actionBtn, command, processURL, messageView, callback, sendResponse) {
 ...
 * @param fallback any
 * */
export function paginationDriver(viewMode, url, extractTo, fallback, callback) {
 ...
 * @param callback any
 * */
export function checkInputDataAbility(url, dataObject, callback) {
    return sendRequest({
        method: "POST", url: url, async: true,
        header: [{ name: "Content-type", value: "application/json;charset=UTF-8" }],
        data: dataObject,
