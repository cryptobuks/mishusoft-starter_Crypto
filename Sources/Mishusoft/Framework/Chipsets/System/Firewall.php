...
class Firewall
{
    ...
    public const firewallConfigFile = PHP_RUNTIME_REGISTRIES_PATH.'firewall.json';
    public const firewallLogFile    = PHP_RUNTIME_REGISTRIES_PATH.'access.logs.firewall.json';

    public const requiredKeys = [
        'status'                             => 'enable',
    ...
    private function loadConfig(): void
    {
         ...
         * */
        if (file_exists(self::firewallConfigFile) === false) {
            FileSystem::write(self::firewallConfigFile, []);
        }

        /*
         * Preparing to check firewall logs file.
         * */
        if (file_exists(self::firewallLogFile) === false) {
            FileSystem::write(self::firewallLogFile, []);
        }

        /*
         * check read permission of configuration file
         * */
        if (is_readable(self::firewallConfigFile) === true) {
            /*
             * check configuration file's content are valid array
             * */
            $oldConfiguration = json_decode(
                file_get_contents(self::firewallConfigFile),
                true,
                512,
                JSON_THROW_ON_ERROR
            );
            ...
            if (is_array(json_decode(file_get_contents(self::firewallConfigFile), true)) === true) {
            ...
            throw new RuntimeException('Read permission denied for'.$this->firewallConfiguration);
        }//end if
    ...
    {
        if (file_exists(self::firewallConfigFile) === true) {
            FileSystem::remove(self::firewallConfigFile);
        }

        FileSystem::write(self::firewallConfigFile, $config);
        ...
        if (is_array($request) === true) {
            foreach ($request as $key => $value) {
                if ((_String::lower($key) === 'host')
                    && _Array::value($this->firewallConfiguration, 'hostname') !== $value
                    && $this->isListed(IP::get()) === false
                ) {
                    $this->actionStatus    = 'blocked';
        ...
        if (empty($item) === false) {
            ...
            foreach ($this->firewallConfiguration as $key => $value) {
                if (is_array($this->firewallConfiguration[$key]) === true
                    ...
                    && in_array($item, $this->firewallConfiguration[$key][$list], true) === true
                ) {
                    return true;
                }
            }
        } else {
            throw new RuntimeException('Item not parsed in '.__METHOD__);
        }
    ...
    private function updateList(string $ip, string $list): void
    {
        if (in_array($ip, $this->firewallConfiguration['ip'][$list], true) === false) {
            ...
            FileSystem::saveToFile(self::firewallConfigFile, _JSON::encodeToString($this->firewallConfiguration));
        }
        ...
        foreach (Http::getErrorsRecord() as /*$errKey =>*/ $details) {
            if (array_key_exists('Description', $details) === true
                ...
                if (Memory::Data('framework')->debug === true) {
                    ...
                    if (array_key_exists('error', $message) and array_key_exists('description', $message['error']) === true) {
    ...
    private static function runtimeFailureUi(string $title, array $message): void
    {
        ...
        $request_address = Browser::getVisitedPage();

        if ((new Browser())->getRequestMethod() === 'OPTIONS') {
            // add welcome not for http options method
            Media::StreamAsJson(['message' => ['type' => 'success', 'contents' => "The HTTP OPTIONS method requests permitted to communicate for $request_address."]]);
            ...
            if (Memory::Data('framework')->debug) {
                ...
                Ui::HtmlInterface(
                    $title,
                    function ($html, $head) use ($message, $request_address, $request_method, $title) {
    ...
    {
        $logs = [];
        if (FileSystem::IsReadable(self::firewallLogFile)) {
            if (strlen(file_get_contents(self::firewallLogFile)) !== 0) {
                ...
                if (count($logs) !== 0) {
                    if (array_key_exists($this->actionStatus, $logs)) {
                        if (is_array($logs[$this->actionStatus]) and array_key_exists(IP::get(), $logs[$this->actionStatus])) {
                            if (is_array($logs[$this->actionStatus][IP::get()]) and array_key_exists($this->browser->getBrowserNameFull(), $logs[$this->actionStatus][IP::get()])) {
                                if (is_array($logs[$this->actionStatus][IP::get()][$this->browser->getBrowserNameFull()]) and !array_key_exists(Time::getToday(), $logs[$this->actionStatus][IP::get()][$this->browser->getBrowserNameFull()])) {
                                    if (FileSystem::IsWriteable(self::firewallLogFile)) {
                                        FileSystem::saveToFile(
                                            self::firewallLogFile,
                                            json_encode(
                                                array_merge(
                                                    $logs,
                                                    [
                                                        $this->actionStatus => array_merge(
                                                            ...
                                                            [
                                                                IP::get() => array_merge(
                                                                    ...
                                                                    [
                                                                        $this->browser->getBrowserNameFull() => array_merge($logs[$this->actionStatus][IP::get()][$this->browser->getBrowserNameFull()], $this->getNewVisitorTimeBased()),
                                                                    ]
                                                                ),
                                                            ]
                                                        ),
                                            ...
                                            array_merge(
                                                ...
                                                [
                                                    $this->actionStatus => array_merge(
                                                        $logs[$this->actionStatus],
                                                        [
                                                            IP::get() => array_merge($logs[$this->actionStatus][IP::get()], $this->getNewVisitorBrowserBased()),
                                                        ]
                                                    ),
                                                ]
                                            )
                    ...
                    if (FileSystem::IsWriteable(self::firewallLogFile)) {
                        FileSystem::saveToFile(self::firewallLogFile, json_encode([$this->actionStatus => $this->getNewVisitorIPBased()]));
                    }
                }//end if
            } else {
                if (FileSystem::IsWriteable(self::firewallLogFile)) {
                                    ...
                                    if (array_key_exists("$status-device-count-down-time", $this->firewallConfiguration)) {
                                        ...
                                        if (!empty($this->firewallConfiguration["$status-device-count-down-time"])) {
                                            ...
                                            if ($this->last_visit_duration >= $this->firewallConfiguration["$status-device-time-limit"]) {
                                                ...
                                                FileSystem::saveToFile(self::firewallConfigFile, json_encode($this->firewallConfiguration));
                                            } //end if
                                        ...
                                        if (FileSystem::IsWriteable(self::firewallConfigFile)) {
        ...
        if (!empty($viewMode) and is_string($viewMode)) {
            if (Network::requestHeader('HTTP_SEC_FETCH_MODE') === 'cors') {
                ...
                Ui::HtmlInterface(
                    ...
                    function ($html, $head) use ($lb, $status, $title, $request_address, $request_method) {
