...
class Network
{
    ...
    {
        return array_key_exists(_String::upper($argument), $_SERVER) ? $_SERVER[_String::upper($argument)] : 'Unavailable';
    }
     ...
     */
    public static function getServerNameVersion()
    {
        if (preg_match("/apache/i", _String::lower($_SERVER["SERVER_SOFTWARE"]))) {
            $server = str_replace('/', ' ', substr($_SERVER["SERVER_SOFTWARE"], 0, 20));
        } else {
            if (preg_match("/litespeed/i", _String::lower($_SERVER["SERVER_SOFTWARE"]))) {
