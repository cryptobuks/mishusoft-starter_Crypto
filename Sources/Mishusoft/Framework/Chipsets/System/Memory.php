...
class Memory
{
    ...
    private static function loadCoreDefinedConstants()
    {
        ...
        //define('MS_COMPOSER_PATH', PHP_RUNTIME_ROOT_PATH . 'Composer' . DIRECTORY_SEPARATOR);
        ...
        if (is_readable(join([PHP_RUNTIME_SYSTEM_PATH, "Registries/framework.install.json"]))) {
            define('BaseURL', self::Data('framework', ["file" => join([PHP_RUNTIME_SYSTEM_PATH, "Registries/framework.install.json"])])->host->url);
        } else {
     ...
     * @return mixed
     */
    public static function Data(string $carrier = 'memory', array $options = array()): mixed
    {
        //preOutput(debug_backtrace());
        switch ($carrier) {
            case "config":
                $format = array_key_exists("format", $options) ? $options["format"] : "object";
                $default = array_key_exists("default", $options) ? $options["default"] : array("status" => "no_data");
    ...
    {
        return Stream::readFromFile($filename, function ($contents) use ($default, $format) {
    ...
    {
        /*Autoload::log("Want to read $filename.");
        Autoload::log("Checking for read permission $filename.");*/
        if (is_readable(stream_resolve_include_path($filename))) {
            /*Autoload::log("Retrieve data form $filename.");*/
            self::Read(json_decode(file_get_contents(stream_resolve_include_path($filename))));
        } else {
            /*Autoload::log("Unable to read local file $filename.");
            Autoload::log("Retrieve data form backup file backup file.");*/
    ...
    private static function Read(object $framework)
    {
        if (!empty($framework)) {
            ...
            define('DEFAULT_SYSTEM_THEME', $framework->preset->theme); /*alias of default system layout*/
            ...
            /*Autoload::log("Memory is corrupted.");*/
