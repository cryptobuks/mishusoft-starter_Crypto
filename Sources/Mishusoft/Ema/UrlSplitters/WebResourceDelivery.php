                        ...
                        if (false !== strpos(end($array), "-")) {
                            ...
                            if (preg_match("[.$ext]", $expected)) {
                                ...
                                if (file_exists(Media::getMediaPath('logos/' . Memory::Data()->preset->logo))) {
                                    Media::StreamOriginalFile(
                                        Media\Image::resize(Media::getMediaPath('logos/' . Memory::Data()->preset->logo),
                                            $width, $height, MS_PRIVATE_MEDIA_PATH . 'logos' . DIRECTORY_SEPARATOR . $filename)
                                    );
                                }
                            } else {
                                Media::StreamOriginalFile(
                                    Media\Image::resize(Media::getMediaPath('logos/' . Memory::Data()->preset->logo),
                                        16, 16, MS_PRIVATE_MEDIA_PATH . 'logos' . DIRECTORY_SEPARATOR . $filename)
                                );
                            }
                        } else {
                            Firewall::runtimeFailure('Not Found', [
