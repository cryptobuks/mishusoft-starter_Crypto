...
(function () {
    ...
    if (_Array::value(System::$event, "type") === "success" && _Array::value(System::$event, "message") === "ok") {
        ...
        if (!in_array(_Array::value(System::$event, "message"), System::getExcludeErrors())) {
            Ui::HtmlInterface(ucfirst(_Array::value(System::$event, "type")), function ($html, $head) {
                /*add meta tags*/
                ...
                $template = Ui::element(
                    Ui::element(
                        Ui::element($html, 'body', ['id' => 'error']),/*set id attribute for body*/
                    ...
                    "ms-app-content", ["style" => "display: block;margin: 0;padding: 0;text-align: left;border: 1px solid #f22b08;-webkit-border-radius: 5px;border-radius: 5px"]
                );
            ...
            Ui::HtmlInterface("Loading...", function ($html, $head, $title) {

                /*add meta tags*/
                Ui::elementList($head, [
                    'meta' => [
                        ['id' => 'mishusoft-web-root', 'name' => 'root', 'content' => Memory::Data('framework')->host->url],
                        ['id' => 'runtime-server-name-version', 'name' => 'server-name-version', 'content' => Network::getServerNameVersion()],
                        ['id' => 'runtime-php-version', 'name' => 'php-version', 'content' => phpversion()],
                        ['id' => 'runtime-host-name', 'name' => 'host-name', 'content' => array_key_exists('static_hostname', (new Device)->hostnamectl()) ? (new Device)->hostnamectl()['static_hostname'] : php_uname()],
                        ['id' => 'runtime-host-ip', 'name' => 'host-ip', 'content' => $_SERVER['SERVER_ADDR']],
                        ['id' => 'runtime-host-os', 'name' => 'host-os', 'content' => array_key_exists('pretty_name', (new Device)->osRelease()) ? (new Device)->osRelease()['pretty_name'] : PHP_OS],
                        ['id' => 'runtime-host-architecture', 'name' => 'host-architecture', 'content' => array_key_exists('architecture', (new Device)->hostnamectl()) ? "(" . (new Device)->hostnamectl()['architecture'] . ")" : ""]
                    ],
                ]);
                ...
                Ui::elementList($body, [
                    'script' => [
                        ['type' => 'application/javascript', 'text' => 'let _root_ = \'' . Memory::Data("framework")->host->url . '\';function fixWindowSize(){if(window.innerHeight < "700") {document.body.style = "height:700px;";} else if(window.innerHeight > "700") {document.body.style = "height:"+window.innerHeight + "px";} else {if(window.innerHeight > "1024") {document.body.style = "height:1024px;";}}} window.addEventListener("resize", fixWindowSize);window.addEventListener("load", fixWindowSize);'],
                        ['type' => 'application/javascript', 'src' => Media::toDataUri("js_installer.js", "remote")/*.join([Memory::Data("framework")->host->url . 'libraries/js/installer.js'])*/],
                        ['type' => 'application/javascript', 'src' => Media::toDataUri("js_app-js-framework-v4.js", "remote")]
                    ],
                ]);
            });
        }
