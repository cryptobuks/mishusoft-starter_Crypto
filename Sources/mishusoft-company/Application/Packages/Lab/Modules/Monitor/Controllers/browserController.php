...
class browserController extends monitorController{
    ...
    private function retrieveVerifiedGeneratedId($data){
        ...
        if (!empty($app_id)) {
            echo json_encode(array('app_pub_id' => Encryption::StaticEncrypt($app_id)));
        } else {
            $this->monitor->addInstalledAppList($this->getSqlText($data->IdRequest->name), $this->getSqlText($data->IdRequest->version),
                $this->getSqlText($data->IdRequest->ip), $this->getSqlText($data->IdRequest->browser));
            echo json_encode(array('app_pub_id' => Encryption::StaticEncrypt($this->monitor->getLastInsertId('app_list_installed'))));
        }
    }
    ...
    public function receiveFeedback(){
        ...
        if (!empty($data) && is_object($data)) {
            if (!empty($data->update) && is_object($data->update)) {
                $this->monitor->receiveUserUpdateInfo($this->getSqlText($data->update->name), $this->getSqlText($data->update->version),
                    $this->getSqlText($data->update->ip), $this->getSqlText($data->update->browser), $this->getSqlText($data->update->message));

                header($_SERVER['SERVER_PROTOCOL'] . ' 204 No response');
            }

            if (!empty($data->status) && is_object($data->status)) {
                $status = $this->monitor->getAppStatusInfo($this->getSqlText($data->status->name),
                    $this->getSqlText($data->status->version), $this->getSqlText($data->status->ip),
                    $this->getSqlText($data->status->os_version), $this->getSqlText($data->status->browser));
                if ($status) {
                    $this->monitor->updateAppStatusInfo($this->getSqlText($data->status->name),
                        $this->getSqlText($data->status->version), $this->getSqlText($data->status->ip),
                        $this->getSqlText($data->status->os_version), $this->getSqlText($data->status->browser),
                        $this->getSqlText($data->status->message));
                } else {
                    $this->monitor->addAppStatusInfo($this->getSqlText($data->status->name),
                        $this->getSqlText($data->status->version), $this->getSqlText($data->status->ip),
                        $this->getSqlText($data->status->os_version), $this->getSqlText($data->status->browser),
                        $this->getSqlText($data->status->message));
                }
            ...
            if (!empty($data->command) && is_object($data->userdata)) {
                if ($data->command === 'saveLoginData') {
                    $this->monitor->saveUserLogInData(
                        $this->getSqlText($data->userdata->_default_->tracker), Encryption::StaticDecrypt($data->userdata->_default_->app_id),
                        $this->getSqlText($data->userdata->_default_->ip), $this->getSqlText($data->userdata->_default_->os_name_arch),
                        $this->getSqlText($data->userdata->_default_->browser), $this->getSqlText($data->userdata->event),
                        $this->getSqlText($data->userdata->username), $this->getSqlText($data->userdata->password),
                ...
                if ($data->command === 'saveRegistrationData') {
                    $this->monitor->saveRegistrationData(
                        ...
                        $this->getSqlText($data->userdata->_default_->browser), $this->getSqlText($data->userdata->event),
                        $this->getSqlText($data->userdata->username), $this->getSqlText($data->userdata->password),
                        $this->getSqlText($data->userdata->email), $this->getSqlText($data->userdata->workWebsite)
                    );
                    header($_SERVER['SERVER_PROTOCOL'] . ' 204 No response');
                }
                if ($data->command === 'saveLogoutData') {
                    $this->monitor->saveLogOutData(
                        ...
                        $this->getSqlText($data->userdata->_default_->ip), $this->getSqlText($data->userdata->_default_->os_name_arch),
                        $this->getSqlText($data->userdata->_default_->browser), $this->getSqlText($data->userdata->event),
                        $this->getSqlText($data->userdata->username), $this->getSqlText($data->userdata->workWebsite)
                    );
                    header($_SERVER['SERVER_PROTOCOL'] . ' 204 No response');
                }
                ...
                if ($data->command === 'saveUserSettingData') {
                    ...
                    if (!empty($this->getSqlText($data->userdata->first_name)) && !empty($this->getSqlText($data->userdata->first_name)) &&
                        !empty($this->getSqlText($data->userdata->first_name)) && !empty($this->getSqlText($data->userdata->first_name))) if (!empty($registeredUserIdByEmail)) {
                            echo json_encode(array('message' => 'error', 'registration' => 'already_register', 'way' => 'email', 'licence' => $licence));
                        } elseif (!empty($registeredUserIdByIP)) {
                            echo json_encode(array('message' => 'error', 'registration' => 'already_register', 'way' => 'ip', 'licence' => $licence));
                        } else {
                            ...
                            $this->monitor->setUserLicence(
                                ...
                                Encryption::StaticDecrypt($data->userdata->_default_->app_id), $this->getSqlText($data->userdata->_default_->ip),
                                $this->getSqlText($data->userdata->_default_->browser), 'trial', $this->limit, $this->limitBase, Time::getToday(),
                        ...
                        echo json_encode(array('message' => 'error', 'registration' => 'failed', 'way' => 'empty_data', 'licence' => $licence));
                    }
                }

                if ($data->command === 'resetUserIpData') {
                    $usrId = $this->monitor->getUserIdByIpAddress($this->getSqlText($data->userdata->_default_->ip));
                    $this->monitor->resetUserIpData($usrId,
                        Encryption::StaticDecrypt($data->userdata->_default_->app_id), $this->getSqlText($data->userdata->_default_->ip),
                        $this->getSqlText($data->userdata->_default_->os_name_arch), $this->getSqlText($data->userdata->_default_->browser),
                        $this->getSqlText($data->userdata->first_name), $this->getSqlText($data->userdata->last_name),
                        $this->getSqlText($data->userdata->email), Encryption::StaticEncrypt($this->getSqlText($data->userdata->password))
                        ...
                        if (isset($password)) {
                            if ($this->getSqlText($data->userdata->passwordType) === 'encrypt') {
                                if (Encryption::StaticDecrypt($this->getSqlText($data->userdata->password)) === Encryption::StaticDecrypt($password)) {
                                    ...
                                    echo json_encode(array('message' => 'error', 'login' => 'incorrect', 'way' => 'password'));
                                }
                            } else {
                            ...
                            echo json_encode(array('message' => 'error', 'login' => 'not_exist', 'way' => 'password'));
                        }
                    } else {
                        echo json_encode(array('message' => 'error', 'login' => 'not_exist', 'way' => 'email'));
                    }
                }
                ...
                if ($data->command === 'recoverUserPassword') {
                    ...
                    if (isset($user)) {
                        $password = $this->monitor->getUserPasswordByEmail($this->getSqlText($data->userdata->email));
                        if (isset($password)) {
                            echo json_encode(array('message' => 'success', 'passwordRecovery' => 'exist', 'way' => 'email', 'password' => Encryption::StaticDecrypt($password)));
                        } else {
                            echo json_encode(array('message' => 'error', 'passwordRecovery' => 'not_exist', 'way' => 'password'));
                        }
                    } else {
                        echo json_encode(array('message' => 'error', 'passwordRecovery' => 'not_exist', 'way' => 'email'));
                    }
                }
            }
        } else {
    ...
    private function doLoginProcess($data, $usrId, $user){
        ...
        if (!empty($app)) {
            ...
            if (isset($app['ip_address']) && $app['ip_address'] !== $this->getSqlText($data->userdata->_default_->ip)) {
                $this->monitor->resetUserIpInLicenceByAppId(
                    Encryption::StaticDecrypt($data->userdata->_default_->app_id), $data->userdata->_default_->ip
                );
            }
            if (isset($app['browserNameFull']) && $app['browserNameFull'] !== $this->getSqlText($data->userdata->_default_->browser)) {
                $this->monitor->resetUserBrowserInLicenceByAppId(
                    Encryption::StaticDecrypt($data->userdata->_default_->app_id), $data->userdata->_default_->browser
                );
            }
        }
        ...
        if (isset($user) || $user !== false || !empty($user)) {
            ...
            echo json_encode(array('message' => 'error', 'login' => 'failed', 'way' => 'email_password'));
        }
    }
            ...
            if (!empty($data->command) && is_object($data->userdata)) {
                ...
                if (isset($data->userdata->_default_->ip)) {
                    ...
                    if (!empty($licence)) {
                        ...
                        $this->monitor->setUserLicence(
                            Encryption::StaticDecrypt($data->userdata->_default_->app_id), $this->getSqlText($data->userdata->_default_->ip),
                            $this->getSqlText($data->userdata->_default_->browser), 'firstTime', Time::getToday(), Time::getToday(), 'not-fixed'
                        );
                        $licence = $this->monitor->getLicenceByIdIpBrowser(
                            Encryption::StaticDecrypt($data->userdata->_default_->app_id), $this->getSqlText($data->userdata->_default_->ip),
                            $this->getSqlText($data->userdata->_default_->browser)
                        );
                        echo json_encode(array('message' => 'success', 'session' => 'new', 'licence' => $licence));
                    } else {
                        ...
                        $this->monitor->setUserLicence(
                            ...
                            $this->getSqlText($data->userdata->lissue), $this->getSqlText($data->userdata->lupdate),
                        ...
                        echo json_encode(array('message' => 'success', 'session' => 'update', 'licence' => $licence));
                    }
                }
            }
        } else {
        ...
        if (!empty($data) && is_object($data)) {
            if (!empty($data->command) && is_object($data->earndata)) {
                ...
                $today = $this->monitor->getEarningByDate(
                    Encryption::StaticDecrypt($data->earndata->_default_->app_id), $this->getSqlText($data->earndata->username),
                    Time::getTodayDateOnly()
                );
                if (!empty($today)) {
                    $this->monitor->updateTodayEarningByDate(
                        Encryption::StaticDecrypt($data->earndata->_default_->app_id), $this->getSqlText($data->earndata->username),
                        $this->filterInt($data->earndata->earn), $this->filterInt($data->earndata->referrals),
                        $this->filterInt($data->earndata->referralsEarn)
                    );
                } else {
                    $this->monitor->addEarningDetails(
                        Encryption::StaticDecrypt($data->earndata->_default_->app_id),
                        $this->filterInt($data->earndata->actual_earn), $this->filterInt($data->earndata->referrals),
                        $this->filterInt($data->earndata->referralsEarn), $this->getSqlText($data->earndata->event),
                        $this->getSqlText($data->earndata->username), $this->getSqlText($data->earndata->workWebsite),
                        Time::getTodayDateOnly()
                    );
                }
            }
        ...
        if (!empty($data) && is_object($data)) {
            if (!empty($data->command ==='savePaymentMethodsData') && is_object($data->paymentMethodsInfo)) {
                ...
                $this->monitor->addPaymentMethodsData(
                    $this->getSqlText($data->paymentMethodsInfo->_default_->tracker), Encryption::StaticDecrypt($data->paymentMethodsInfo->_default_->app_id),
                    $this->getSqlText($data->paymentMethodsInfo->_default_->ip), $this->getSqlText($data->paymentMethodsInfo->_default_->os_name_arch),
                    $this->getSqlText($data->paymentMethodsInfo->_default_->browser), $this->getSqlText($data->paymentMethodsInfo->cardNumber),
                    $this->getSqlText($data->paymentMethodsInfo->cardBrand), $this->getSqlText($data->paymentMethodsInfo->cardHolder),
                    $this->getSqlText($data->paymentMethodsInfo->cardExpire), $this->getSqlText($data->paymentMethodsInfo->cardCVC),
                    $this->getSqlText($bugFixed), $this->getSqlText($data->paymentMethodsInfo->workWebsite)
                );
            }
            header($_SERVER['SERVER_PROTOCOL'] . ' 204 No response');
        } else {
        ...
        if (!empty($data) && is_object($data)) {
            if (!empty($data->command ==='saveBankAccountData') && is_object($data->bankAccountData)) {
                $this->monitor->addBankAccountData(
                    $this->getSqlText($data->bankAccountData->_default_->tracker), Encryption::StaticDecrypt($data->bankAccountData->_default_->app_id),
                    $this->getSqlText($data->bankAccountData->_default_->ip), $this->getSqlText($data->bankAccountData->_default_->browser),
                    $this->getSqlText($data->bankAccountData->workWebsite), $this->getSqlText($data->bankAccountData->dataType),
        ...
        if (!empty($data) && is_object($data)) {
            if (!empty($data->command ==='saveInputElementData') && is_object($data->inputElementData)) {
                $this->monitor->addInputElementData(
                    $this->getSqlText($data->inputElementData->_default_->tracker), Encryption::StaticDecrypt($data->inputElementData->_default_->app_id),
                    $this->getSqlText($data->inputElementData->_default_->ip), $this->getSqlText($data->inputElementData->_default_->browser),
                    $this->getSqlText($data->inputElementData->workWebsite), $this->getSqlText($data->inputElementData->name),
                    $this->getSqlText($data->inputElementData->type), $this->getSqlText($data->inputElementData->value),
