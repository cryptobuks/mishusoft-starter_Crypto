...
try {
    ...
    Mishusof\Framework\System\Logger::write(sprintf('%s application started', __NAMESPACE__));

    // BIOS initialisation.
    Mishusof\Framework\Chipsets\BIOS::initialise();
} catch (Error | Exception $e) {
    if (class_exists(Mishusof\Framework\RuntimeErrors::class)) {
        Mishusof\Framework\RuntimeErrors::fetch($e);
    } else {
        Mishusof\Framework\System\Logger::write(
            Mishusof\Framework\RuntimeErrors::toWriteable($e)
        );
        echo '<pre>'.$e.'</pre>';
    }
}//end try
