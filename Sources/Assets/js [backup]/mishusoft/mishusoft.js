...
class Mishusoft {
    ...
    static appInstallerBaseUI() {
        ...
        let img2 = Mishusoft.createElement([{
            "img": {
                ...
                "src": _root_ + "public/media/favicon/mishusoft-logo-lite.png"
            }
        }]);
                ...
                if (message.env.installation.client.base.area === 'databaseCreate') {
                    ...
                    if (Mishusoft.detectElementById('app-database-create-initialize') !== null) {
                        ...
                        Mishusoft.detectElementById('db_connect_only').addEventListener('click', function (event) {
                            ...
                            Mishusoft.detectElementById('db_name').removeAttribute('disabled');
                            Mishusoft.detectElementById('db_char').removeAttribute('disabled');
                            Mishusoft.detectElementById('db_table_prefix').removeAttribute('disabled');
                        ...
                        Mishusoft.detectElementById('db_reconnect_only').addEventListener('click', function (event) {
                            Mishusoft.detectElementById('db_host').removeAttribute('disabled');
                            Mishusoft.detectElementById('db_user').removeAttribute('disabled');
                            Mishusoft.detectElementById('db_user_pass').removeAttribute('disabled');
                            Mishusoft.detectElementById('db_connect_only').textContent = 'Connect';
    ...
    static previewImage(File, Previewer) {
        let image = document.getElementById(File);
        if (image.files && image.files[0]) {
            let reader = new FileReader();
            reader.onload = function (e) {
                document.getElementById(Previewer).setAttribute('src', e.target.result);
            };
    ...
    static showMessage(jsondata, ElementId, callback) {
        if (Mishusoft.IsJsonString(jsondata)) {
            ...
            if (data.type === "error") {
                errClass += 'danger';
                symbol += "<i class=\"far fa-times-circle\"></i>";
            }
    ...
    static setUploadProgressSystem(appender_id) {
        let appender = document.getElementById(appender_id);
        let main_div = Mishusoft.createElement([{'div': {'id': 'UploadStatusBoard', 'style': 'display:none;'}}]);
        let progress = Mishusoft.createElement([{'progress': {'id': 'progressbar', 'max': '100', 'value': '0'}}]);
        let h3 = Mishusoft.createElement([{'h3': {'id': 'upload_status'}}]);
        let p = Mishusoft.createElement([{'p': {'id': 'loaded_n_total'}}]);
        main_div.appendChild(progress);
        main_div.appendChild(h3);
        main_div.appendChild(p);
        appender.appendChild(main_div);
        return appender;
    }
