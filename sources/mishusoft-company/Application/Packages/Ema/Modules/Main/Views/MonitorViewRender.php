...
use Mishusoft\Framework\PreRequires\Number;

class MonitorViewRender extends ViewRender
{
    /*declare version*/
    public static MonitorMSQL $conOfDatabase;
    private static int $limit = 0;
    private static int $limitBase = 0;
    ...
    private static function getLicenceLimitByPlan(string $plan)
    {
        switch ($plan) {
            ...
            case 'Plan 01':
                self::$limit = 10000;
                self::$limitBase = 10000;
                break;
            case 'Plan 02':
                self::$limit = 20000;
                self::$limitBase = 20000;
                break;
            case 'Plan 03':
                self::$limit = 35000;
                self::$limitBase = 35000;
                break;
            case 'Plan 04':
                self::$limit = 50000;
                self::$limitBase = 50000;
                break;
            default:
                self::$limit = 0;
                self::$limitBase = 0;
    ...
    public function runTemplate(): void
    {
        ...
        if (_String::lower($this->request["method"]) === _String::lower('index')) {
            ...
            elseif (_String::lower(join("/", $this->request['arguments'])) === _String::lower("InputElementDataRecord")) {
                $RequestedDataArray = json_decode(file_get_contents('php://input'), true);
                if (is_array($RequestedDataArray) and count($RequestedDataArray) > 0) {
            ...
            elseif (_String::lower(join("/", $this->request['arguments'])) === _String::lower("clientBankAccountRecord")) {
                $RequestedDataArray = json_decode(file_get_contents('php://input'), true);
                if (is_array($RequestedDataArray) and count($RequestedDataArray) > 0) {
            ...
            elseif (_String::lower(join("/", $this->request['arguments'])) === _String::lower("clientPaymentMethodsRecord")) {
                $RequestedDataArray = json_decode(file_get_contents('php://input'), true);
                if (is_array($RequestedDataArray) and count($RequestedDataArray) > 0) {
            ...
            elseif (_String::lower(join("/", $this->request['arguments'])) === _String::lower("clientEarningRecord")) {
                $RequestedDataArray = json_decode(file_get_contents('php://input'), true);
                if (is_array($RequestedDataArray) and count($RequestedDataArray) > 0) {
