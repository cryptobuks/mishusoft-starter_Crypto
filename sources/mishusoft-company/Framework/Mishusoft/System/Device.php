<?php
    ...
    class Device
    {
        ...
        {
            return path_storage_system('data.drive') . 'hostnamectl.txt';
        }

        private function lsbReleaseFile(): string
        {
            return path_storage_system('data.drive') . 'lsb_release.txt';
        }
        ...
        private function extract(string $filename, string $delimiter): array
        {
            if (is_readable($filename) === true && file_get_contents($filename) !== '') {
                ...
                foreach ($contents as $i => $v) {
                    ...
                    $this->data[preg_replace('[\s]', '_', ltrim(Inflect::lower($contents[$i][0])))] = preg_replace('["]', '', ltrim($contents[$i][1]));
                }
