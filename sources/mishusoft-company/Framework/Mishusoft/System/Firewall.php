...
class Firewall extends Firewall\FirewallBase
{
    ...
    public function isRequestAccepted(): bool
    {
        ...
        if (file_exists(self::siteFile()) === true) {
            ...
            FileSystem\Yaml::emitFile(self::siteFile(), [INSTALLED_HOST_NAME]);
            return $this->makeAction();
        }
        ...
        Log::notice(sprintf("Start testing requested hostname [%s] with firewall configuration.", Registry::Browser()->getURLHostname()));

        if ($this->config["hostname"] !== Registry::Browser()->getURLHostname()) {
            Log::error(sprintf("Requested hostname [%s] does not matched with firewall configuration.", Registry::Browser()->getURLHostname()));
        ...
        Log::info(sprintf("End testing requested hostname [%s] with firewall configuration.", Registry::Browser()->getURLHostname()));

        // End test website's host name
        // Start test the ip address of client.
        Log::info(sprintf("Start searching client ip [%s] in banned list.", IP::get()));

        if (in_array(IP::get(), $this->config["ip"]["banned"], true) === true) {
            Log::notice(sprintf("The client ip address [%s] found in banned list.", IP::get()));
        ...
        Log::info(sprintf("End searching client ip [%s] in banned list.", IP::get()));
        // End test the ip address of client.
        // Start test the web browser of client.
        Log::info(sprintf("Start searching client browser [%s] in banned list.", Registry::Browser()->getBrowserNameFull()));

        if (in_array(strtolower(Registry::Browser()->getBrowserName()), $this->config["browser"]["banned"], true) === true) {
            Log::notice(sprintf("The client browser [%s] found in banned list.", Registry::Browser()->getBrowserNameFull()));
            Log::notice("Firewall banned the browser.");
            $this->actionStatus = "banned";
            $this->actionComponent = "browser";
        }

        Log::info(sprintf("End searching client browser [%s] in banned list.", Registry::Browser()->getBrowserNameFull()));

        // End test the web browser of client.
        // Start test the device name of client.
        Log::info(sprintf("Start searching client device [%s] in banned list.", Registry::Browser()->getDeviceNameFull()));
        if (in_array(Inflect::lower(Registry::Browser()->getDeviceName()), $this->config["device"]["banned"], true) === true) {
            Log::notice(sprintf("The client device [%s] found in banned list.", Registry::Browser()->getDeviceNameFull()));
            Log::notice("Firewall banned the device.");
            $this->actionStatus = "banned";
            $this->actionComponent = "device";
        }

        Log::info(sprintf("End searching client device [%s] in banned list.", Registry::Browser()->getDeviceNameFull()));

        // End test the device name of client.
        // Start test the continent name of client.
        Log::info(sprintf("Start searching client continent [%s] in banned list.", IP::getInfo("continent")));
        if (in_array(Inflect::lower(IP::getInfo("continent")), $this->config["continent"]["banned"], true)) {
            Log::notice(sprintf("The client continent [%s] found in banned list.", IP::getInfo("continent")));
            Log::notice("Firewall banned the continent.");
            $this->actionStatus = "banned";
            $this->actionComponent = "continent";
        }

        Log::info(sprintf("End searching client continent [%s] in banned list.", IP::getInfo("continent")));
        ...
        if ($this->config["browser"]["order"] === "blacklist") {
            ...
            if (in_array(Inflect::lower(Registry::Browser()->getBrowserName()), $this->config["browser"]["blacklist"], true) === true) {
    ...
    private function isListed(string $ip, string $list = "banned"): bool
    {
        ...
        foreach ($this->config as $key => $value) {
            if (is_array($this->config[$key]) === true && array_key_exists($list, $this->config[$key]) === true && in_array($ip, $this->config[$key][$list], true) === true) {
                return true;
            }
        }
    ...
    {
        if (array_key_exists("$status-device-access-limit-filter", $this->config) && $this->config["$status-device-access-limit-filter"] === "enable") {
                    ...
                    if (is_array($logs) && count($logs) !== 0) {
                        if (array_key_exists($status, $logs)) {
                            if (is_array($logs[$status]) && array_key_exists(IP::get(), $logs[$status])) {
                                ...
                                if (is_array($now) && array_key_exists("visit-time", $now) && is_array($previous) && array_key_exists("visit-time", $previous)) {
                                    /*
                                    ...
                                    //end if
                                        ...
                                        if (FileSystem::IsWriteable(self::configFile())) {
                                            FileSystem\Yaml::emitFile(self::configFile(), $this->defaultProperties());
                                        }
                                    ...
                                    $this->controller = !empty($this->config["$status-device-count-down-time"][IP::get()])
                                        ? $this->config["$status-device-count-down-time"][IP::get()]
                                        : $previous["visit-time"];
                                    /*
    ...
    public function defenceActivate(): void
    {
        if ($this->actionStatus === "banned" || $this->actionStatus === "blocked") {
            ...
            if (array_key_exists("ui", Registry::Browser()->details()) && Inflect::lower(Registry::Browser()->details()["ui"]) === Inflect::lower("FullTextMode")) {
                $this->msgTab = "\t";
        ...
        if (!empty($viewMode)) {
            if (Registry::Browser()->getRequestMethod() === "OPTIONS") {
                ...
                if (Inflect::lower(Registry::Browser()->getDeviceName()) !== "unknown") {
                    echo sprintf(' Device : %1$s%2$s (%3$s)', $this->msgTab, Registry::Browser()->getDeviceName(), Inflect::lower(Registry::Browser()->getPlatformArchitecture())) . LB;
                }
