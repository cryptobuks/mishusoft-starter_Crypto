<?php
    ...
    class Classic extends MPM
    {
        ...
        {
            return dFile(configs_data_file('MPM', 'config.classic'));
        }
        ...
        {
            return Memory::getOption('packages.default', 'mpm');
        }//end defaultPackage()
        ...
        {
            return Memory::getOption(sprintf('modules.%1$s.deafult', self::defaultPackage()), 'mpm');
        }//end defaultModule()
        ...
        public static function readConfigure(): bool
        {
            ...
            if (is_readable(self::configFile()) === true) {
                ...
                if (empty(file_get_contents(self::configFile())) === false) {
                    ...
                    $content = parse_msf_file(self::configFile());
                    if ($content !== []) {
...
//                );
            ...
            if (count(self::modulesAll(self::defaultPackage(), ['item' => 'new'])) > 0) {
                throw new Exceptions\RuntimeException\NotFoundException(
                    sprintf(
                        'The module <b>%s</b> are/is not installed.',
                        implode(', ', self::modulesAll(self::defaultPackage(), ['item' => 'new']))
                    )
                );
            }
        }//end moduleMonitor()
                        ...
                        if ($key === 'item') {
                            ...
                            if ($value === 'new') {
                                if (file_exists(Storage::applicationPackagesPath())) {
                                    $dirs = scandir(Storage::applicationPackagesPath());
                                    $dirs = is_array($dirs) ? $dirs : [];
        ...
        {
            if (self::readConfigure()) {
                return self::$content['modules'][$packageName]['all'][$module_name]['status'] === 'enable';
            }
            return false;
        }//end isEnabledModule()
        ...
        {
            if (self::readConfigure()) {
                return self::$content['modules'][$packageName]['all'][$module_name]['status'] === 'disabled';
            }
            return false;
        ...
        public static function getProperty(string $property, string $package = ''): string
        {
            ...
            if (!empty($package)) {
                $properties = decode_from_json(
                    file_get_contents(self::propertiesFile($package)),
                    JSON_LOOSE_TYPE
                );
