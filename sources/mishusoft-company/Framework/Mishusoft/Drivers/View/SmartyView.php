<?php
    ...
    class SmartyView extends SmartyBC
    {
        ...
        public function render(string $view, string $item = '', bool $noLayout = false): void
        {
            ...
            $layoutParams = [
                'publicCSS'                  => getenv('BASE_URL') . 'assets' . DS . 'css' . DS,
                /*
                ...
                'publicFonts'                => getenv('BASE_URL') . 'assets' . DS . 'webfonts' . DS,
                'publicIMG'                  => getenv('BASE_URL') . 'media' . DS . 'images' . DS,
                'publicJS'                   => getenv('BASE_URL') . 'assets' . DS . 'js' . DS,
                /*
                ...
                'publicJSPlugin'             => getenv('BASE_URL') . 'assets' . DS . 'js' . DS . 'plugin' . DS,
                /*
                ...
                // 'favicon' => $favicon,
                'profilePhotosFolder'        => path_storage_authorized('users.profiles'),
                'uploads'                    => Storage::mediaPath(),
                'root'                       => getenv('BASE_URL'),
                'mishusoft_session_validity' => Session::get('auth'),
                'current_host_name'          => Network::getValOfSrv('SERVER_NAME'),
                'configs'                    => [
                    'app_name'    => Memory::getConstant('DEFAULT_APP_NAME'),
                    'app_slogan'  => Memory::getConstant('DEFAULT_APP_DESCRIPTIONS'),
                    'app_author'  => Memory::getConstant('DEFAULT_APP_AUTHOR'),
                    'app_company' => Memory::getConstant('DEFAULT_APP_COMPANY_NAME'),
                ],
            ];
        ...
        public function setJs(array $js)
        {
            if (count($js)) {
                ...
                throw new NotFoundException(
                    'JavaScript Plugin file not found or error while loading JavaScript files '
                    . implode(',', $this->_js)
                );
            }
        }//end setJs()
