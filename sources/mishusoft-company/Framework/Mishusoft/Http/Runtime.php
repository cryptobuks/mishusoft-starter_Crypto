...
class Runtime
{
    ...
    public static function hostUrl(): string
    {
        if (empty(self::$hostUrl)) {
            self::$hostUrl = data("framework")->host->url;
        }
                ...
                str_replace(
                    ...
                    str_replace(
                        str_replace(
                            DS,
                            "_",
                            Storage::applicationWebDirectivePath()
                        ),
                        "",
    ...
    public static function actualUrl(
        ...
        $url = "";
        if (
            count(Arr::cleanArray(self::update($_GET, $message), ["url"])) > 0
        ) {
            foreach (
                Arr::cleanArray(self::update($_GET, $message), ["url"])
                as $key => $value
            ) {
                if (
                    array_key_last(
    ...
    {
        return array_merge(
            Arr::cleanArray($array, ["error", "success", "notify"]),
            [
    ...
    public static function abort(int $status, string|array ...$details): void
    {
        ...
        if (array_key_exists("caption", $message["debug"])) {
            ...
            $messageDetails["debug"]["description"] =
