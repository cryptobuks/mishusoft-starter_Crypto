<?php
    ...
    class SystemPublicInteractionService extends System\Base
    {
        ...
        /**
         * @return SystemPublicInteractionService
        ...
        private static function initializeSecurity(string $status = 'Installing'): void
        {
            $data = [
                'app'    => [
                    ...
                    'author'            => Memory::getConstant('DEFAULT_APP_AUTHOR'),
        ...
        public static function getRequiresFile(string $filename, string $dirname = ''): string
        {
            ...
            $md5HostName           = md5(get_http_host_name());
            $encryptHostName       = Encryption::static(get_http_host_name());
            self::$configDir       = configs_data_directive('MPM');
            self::$configServerDir = self::$configDir . $md5HostName . DS;
            if (empty($dirname) === false) {
                self::$setupFile = dFile(self::$configServerDir . $dirname . DS . $encryptHostName);
            }

            // Application security.
            self::$appSecurityDirectory      = self::$configDir . 'Security' . DS;
            self::$appSecurityFileName       = ucfirst(Memory::getConstant('DEFAULT_APP_NAME')) . 'ApplicationSecurity.lock';
            self::$appSecurityBackupFileName = ucfirst(Memory::getConstant('DEFAULT_APP_NAME')) . 'ApplicationSecurity.lock.cache';
            ...
            self::$appSecurityFileBackupDirectory = path_storage_system('resettable') . 'Backups' . DS;
        ...
        private static function getProgressedSystemStatus(string $appSecurityFile): array
        {
            if ((int)((disk_free_space(RUNTIME_ROOT_PATH) / 1024) / 1024) > 50) {
                if (Memory::getOption('config.database.activation', 'mpm') === true) {
                    ...
                    if (file_exists($appSecurityFile) && !empty(file_get_contents($appSecurityFile))) {
                        ...
                        $security = decode_from_json(file_get_contents($appSecurityFile));
                        ...
                        if (is_object($security)) {
                            ...
                            if (file_exists(self::$appSecurityFileBackup) && !empty(file_get_contents(self::$appSecurityFileBackup))) {
                                if (copy(self::$appSecurityFileBackup, self::$appSecurityFile)) {
                                    self::checkSystemConfiguration();
                                }
                            } else {
                                self::$event = [
            ...
            if (file_exists(self::getRequiresFile('CONFIG_DIR_PATH')) and file_exists(self::getRequiresFile('CONFIG_SERVER_DIR_PATH'))) {
                if (file_exists(self::getRequiresFile('CONFIG_PROPERTIES_FILE_PATH'))) {
                    ...
                    if ($properties !== '' && is_string($properties)) {
                        $properties = decode_from_json($properties, JSON_LOOSE_TYPE);
                                                    ...
                                                    self::makeDbConnectionRequest(
                                                        ...
                                                        function ($connection) use ($properties) {
                                                            $UsersTable = $connection->query(
                                                                'SHOW TABLES FROM `' . Arr::value(
                                                                    self::getDbConfigArgument('db', self::getRequiresFile('SETUP_FILE_PATH', Arr::value($properties, 'default'))),
                                                                    'name'
                                                                ) . "` LIKE '%" . Arr::value(
                                                                    self::getDbConfigArgument('db', self::getRequiresFile('SETUP_FILE_PATH', Arr::value($properties, 'default'))),
                                                                    'prefix'
            ...
            switch (Arr::value(self::$event, 'message')) {
                case 'one-of-module-broken':
                    return self::setRuntimeInstallationEvent(
                        ...
                        '<setup-error> Orphan module exists on module directory. Pleas remove it.</setup-error>'
                    );
                ...
                case 'app-required-database-configure-failed':
                    return self::setRuntimeInstallationEvent(
                        Inflect::ucfirst(Arr::value(self::$event, 'type')),
                        "<setup-error> Framework couldn't configure with database script languages. Please check your database script from <Packages_Root_Directory><Packages_Name>Databases Directory..</setup-error>"
                    );

                case 'app-required-database-connection-failed':
                    return self::setRuntimeInstallationEvent(
                        Inflect::ucfirst(Arr::value(self::$event, 'type')),
                        "<setup-error> Framework couldn't connect with database. Please check your database server and account details.</setup-error>"
                    );
                ...
                case 'app-security-disabled':
                    return self::setRuntimeInstallationEvent(
                        Inflect::ucfirst(Arr::value(self::$event, 'type')),
                        '<setup-error>  Access denied. The Application security is disabled. <br/> The security file not exist. <br/>' . 'File Name: ' . self::getRequiresFile(
                            'SECURITY_FILE_NAME'
                        ) . ' <br/> File Location: ' . self::getRequiresFile('SECURITY_FILE_PATH') . '  </setup-error>'
                    );
            ...
            if (!empty($data) && is_object($data)) {
                if ($data->security_code === 1 && !empty($data->env) && !empty($data->env->installation)) {
                    if ($data->security_code === 1 && !empty($data->env) && !empty($data->env->installation->client->base->area)) {
                        ...
                        if (isset($data->env->installation->client->base->area->database) && $data->env->installation->client->base->area->database !== null && is_object(
                            $data->env->installation->client->base->area->database
                        )) {
                        ...
                        if (isset($data->env->installation->client->base->area->account) && $data->env->installation->client->base->area->account !== null && is_object(
                            $data->env->installation->client->base->area->account
                        )) {
                            ...
                            try {
                                ...
                                self::SetupAppRequiredTables(
                                    self::databaseFile('account'),
                                    Arr::value(self::getDbConfigArgument('db', self::getRequiresFile('SETUP_FILE_PATH', self::getDefaultDb())), 'prefix')
                                );
                                ...
                                if (self::ConfigWebUserQuery(
                                    ...
                                    '1',
                                    '1',
                                    Memory::getConstant('DEFAULT_DATE_OF_BIRTH'),
                                    'male'
                                ) !== null
                                ) {
                                    self::configureUpdate(
                                        [
                                            'account' => [
                                                ...
                                                'role'     => '1',
                                                'status'   => 1,
                                                'dob'      => Memory::getConstant('DEFAULT_DATE_OF_BIRTH'),
                                                'gender'   => 'male',
                                            ],
                                        ]
                                    );
                        ...
                        if (isset($data->env->installation->client->base->area->website) && $data->env->installation->client->base->area->website !== null && is_object(
                            $data->env->installation->client->base->area->website
                        )) {
                            ...
                            try {
                                ...
                                self::SetupAppRequiredTables(
                                    self::databaseFile(Inflect::lower(Memory::getConstant('DEFAULT_APP_NAME'))),
                                    Arr::value(
                                    ...
                                    function ($connection) use ($setupFile, $setupTime, $setupMessage) {
                                        $connection->query(
                                            'INSERT INTO `' . Arr::value(
                                                self::getDbConfigArgument('db', self::getRequiresFile('SETUP_FILE_PATH', self::getDefaultDb())),
                                                'prefix'
                                            ) . "trackSystemUpdate` VALUES (null, 'msu_root', '$setupMessage','$setupFile', '$setupTime');"
                                        );
                                        Storage\Stream::json(
                                            [
                                                'env' => [
        ...
        {
            return self::$databaseFile = implode(DS, [Storage::applicationPackagesPath() . MPM::defaultPackage(), 'Databases', "{$file}.sql"]);
        }//end databaseFile()
        ...
        private static function ConfigWebApp(
            ...
            self::makeDbConnectionRequestAuto(
                function ($connection) use (
                    ...
                    $connection->query(
                        'INSERT INTO `' . $db_prefix . WEB_CONFIG_TABLE . "` VALUES (null, '$name', '$description', '$company', '$doccumentRoot', '$http_host_name', '$http_host_add', '$http_host_ip', '$default_home', '$default_layout', '$icon_remote_dir', '$icon_local_dir', '$favicon');"
                    );
                }
            );
        }//end ConfigWebApp()
