<?php
    ...
    // 1. header area
    // customize : sections,  logo = src, width, height, class, alt
    // customize : menus = items (single/group) {name, link, class} [group alignment, size, class]
    // 2. content area
    // customize : sections, [item {name, class, attributes}]
    // 3. footer area
    // customize : sections (width, hegith, class, etc)
    // customize : copyright = year (current), compnay, addtional text
    ...
    if ($this->templateUse === 'no') {
        ...
        Ui\ProgressiveWebApplication::addMeta(WHO_AM_I);
