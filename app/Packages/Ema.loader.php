                ...
                if (in_array(strtolower($prediction->getController()), $requestedRoute, true) === true) {
                    ...
                    if (is_readable($currentRequestedFile) === true) {
                        ...
                        if (method_exists(new $urlSplitter(), Inflect::lower($prediction->getController())) === true) {
                            ...
                            call_user_func([new $urlSplitter(), Inflect::lower($prediction->getController()),], ['controller' => $prediction->getController(), 'method' => $prediction->getMethod(), 'arguments' => $prediction->getArguments(),]);
                        } else {
                            Logger::write(sprintf('Not found %s form %s.', $urlSplitter . '::' . Inflect::lower($prediction->getController()), $currentRequestedFile));
                            Firewall::runtimeFailure('Not Found', ['debug' => ['file' => $urlSplitter . '::' . Inflect::lower($prediction->getController()), 'location' => Http::browser()::getVisitedPage(), 'description' => 'Your requested url not found!!',], 'error' => ['description' => 'Your requested url not found!!'],]);
                        }
                    } else {
                        Logger::write(sprintf('Not found %s.php from %s.', $requestedClassName, $urlSplitterRootPath));
                        Firewall::runtimeFailure('Not Found', ['debug' => ['file' => sprintf('%s%s.php', $urlSplitterRootPath, $requestedClassName), 'location' => $urlSplitterRootPath, 'description' => 'Required file is not readable!!',], 'error' => ['description' => 'Your requested url not found!!'],]);
                    }
                    exit();
                }
            }
            ...
            foreach (FileSystem::list($requestedHandleDirectory, 'file') as $filename) {
                if (Inflect::lower(BIOS::getFilenameOnly($filename)) === Inflect::lower($prediction->getController())) {
                    ...
                    if (is_readable($currentRequestedFile) === true) {
                        ...
                        call_user_func([new $urlHandler(), 'Response',], ['locale' => $prediction->getLocale(), 'controller' => $prediction->getController(), 'method' => $prediction->getMethod(), 'arguments' => $prediction->getArguments(),]);
                        exit();
                    }
                }
            }
...
if ((Memory::data('framework')->maintenance === true) && $prediction->getController() !== 'maintenance') {
