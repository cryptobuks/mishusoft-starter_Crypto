...
class menuModelWidget extends Model
{
    ...
    {
        $id = (int)$id;
        return ArrayCollection::value($this->db->query("SELECT `name` FROM `" . DbPREFIX . "menu_config_hide_items` WHERE `id` = '{$id}'")->fetch(PdoMySQL::FETCH_ASSOC), 'name');
    }

    public function getPositionNameById($id)
    {
        $id = (int)$id;
        return ArrayCollection::value($this->db->query("SELECT `position` FROM `" . DbPREFIX . "menu_position` WHERE `id` = '{$id}'")->fetch(PdoMySQL::FETCH_ASSOC), 'position');
    }
